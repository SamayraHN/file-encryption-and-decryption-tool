...
class FileEncryptor(QWidget):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("File Encryptor/Decryption Tool")
        ...
        self.encrypt_button.clicked.connect(lambda: self.encrypt_file(
            self.file_path, self.password_input.text()) if hasattr(self, 'file_path') else None)
        ...
        self.decrypt_button.clicked.connect(lambda: self.decrypt_file(
            self.file_path, self.password_input.text()) if hasattr(self, 'file_path') else None)
    ...
    def select_file(self):
        """Opens a file to dialog for selecting the file to encrypt or decrypt."""
        file_dialog = QFileDialog()
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select File to Encrypt/Decrypt")
    ...
    def decrypt_file(self, file_path, password):
        """Decrypts the selected file using AES encryption."""
        key = base64.urlsafe_b64encode(password.ljust(32).encode('utf-8'))
        cipher = Fernet(key)
